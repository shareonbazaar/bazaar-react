...
class StatusOption extends React.Component {
  ...
  render () {
    ...
    const { title, text, buttonText, confirmationText, negatoryText, className, onClick } = this.props;
    return (
      <div className={`status-option ${className || ''}`}>
        ...
        <Button onClick={() => this.setState({showModal: true})} bsStyle="primary" bsSize="large" block>
          {buttonText}
        </Button>
      </div>
    )
