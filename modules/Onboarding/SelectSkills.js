...
class SelectSkills extends React.Component {
  ...
  render() {
    const { title, chosenSkillsTitle, onSkillSelect, onSkillRemove, skills, categories, checkHighLevel } = this.props;
          ...
          categories.map(cat =>
            (
              <div className="skill-select" key={cat._id}>
                <h4>{cat.label.en}</h4>
                {
                  cat._skills.filter(s => s.label.en.toLowerCase().indexOf(inputText) >= 0)
                    .slice(0, NUM_SKILLS_TO_SHOW).map(skill =>
                      (
                        <div
                          key={skill._id}
...
SelectSkills.propTypes = {
  ...
  chosenSkillsTitle: PropTypes.string,
