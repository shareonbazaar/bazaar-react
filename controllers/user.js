...
exports.patchUser = (req, res) => {
  let prevPromise = new Promise(resolve => resolve({}));
  ...
  if (req.file) {
    ...
    const filename = `${req.user._id}.${mimetype.split('/').pop()}`;
    const picUrl = `https://s3.${process.env.AWS_REGION}.amazonaws.com/${process.env.AWS_BUCKET_NAME}/${filename}?time=${new Date().getTime()}`;
    prevPromise = uploadPicture(filename, req.file.buffer, mimetype)
      .then(() => ({ 'profile.picture': picUrl }));
  }
